...
from test.test_importlib.resources.data02 import one
...
"""
    ...
    def test_update_child_check_root(self):
        ...
        for list_of_d, child in zip([[10, 2, 3, 5], [], [5, 20, 3, 2]], vt.get_children()):
            for d in list_of_d:
    ...
    └── 30.0
        ...
        └── 2.0
"""
        assert vt.get_tree_representation(key=lambda node: float(node.get_value())) == expected
