...
class HorizontalRulerUnitTestCase(TestCase):
    def test_components(self):
        ...
        for o in dos:
            ...
            assert o.type.value == "horizontal"
...
class HorizontalRulerTests(TestCase):
    ...
    def test_get_position_markers(self):
        hr = Ruler(type=LineOrientation.HORIZONTAL, length=20, unit_division=3)
...
class HorizontalRulerTestsLabels(TestCase):
    hr1 = Ruler(type=LineOrientation.HORIZONTAL, length=60)
    hr2 = Ruler(
        type=LineOrientation.HORIZONTAL,
        length=120,
        unit_length=20,
...
class VerticalRulerUnitTestCase(TestCase):
    def test_components(self):
        ru = RulerUnit(
        ...
        for o in dos:
            assert isinstance(o, LineSegment)
            assert o.type.value == "vertical"

        assert [cast(LineSegment, o)._length for o in dos] == [4, 4, 4, 4, 4]
        assert [p.y for p in pos] == [0, 4, 8, 12, 16]
...
class VerticalRulerTests(TestCase):
    def test_ruler_units(self):
        vr = Ruler(type=LineOrientation.VERTICAL, length=60)
        assert len(vr._ruler_units) == 6
        for ru in vr._ruler_units:
            assert isinstance(ru, RulerUnit)
            assert ru.type.value == "vertical"
        ru = vr._ruler_units[0]
        assert ru._length == 10
        assert ru._division == 10

    def test_get_positioned_ruler_units(self):
        vr = Ruler(type=LineOrientation.VERTICAL, length=30, unit_division=3)
        rus = vr.get_positioned_ruler_units()
        assert len(rus) == 3
        assert [p.y for p, _ in rus] == [0, 10, 20]
        assert [o for _, o in rus] == vr._ruler_units

    def test_get_position_markers(self):
        hr = Ruler(type=LineOrientation.VERTICAL, length=20, unit_division=3)
        markers = hr.get_positioned_markers()
        assert len(markers) == 6
        assert [round(p.y, 3) for p, _ in markers] == [
            0,
            3.333,
            6.667,
            10,
...
class VerticalRulerTestsLabels(TestCase):

    vr1 = Ruler(type=LineOrientation.VERTICAL, length=60)
    vr2 = Ruler(
        type=LineOrientation.VERTICAL,
        length=120,
        unit_length=20,
    ...
    def test_get_division_size(self):
        assert self.vr1._get_division_size() == 1
        assert self.vr2._get_division_size() == 4

    def test_build_labels(self):
        labels = self.vr1.get_positioned_labels()
        assert len(labels) == 6
        for i, (_, label) in enumerate(labels):
            assert isinstance(label, TextDrawObject)
            assert label.text == str(i)

        labels = self.vr2.get_positioned_labels()
