...
from room_composition.desk_composition_tools.folder_composition_tools_musurgia.musurgia.musurgia.tests.helpers.utils_for_tests import PdfTestCase, create_test_path
...
class TestClip(PdfTestCase):
    ...
    def test_column_page_break(self):
        ...
        clip_area_width = (
            self.pdf.w - self.pdf.l_margin - self.pdf.r_margin - clip_area_left_margin
        )
    ...
    def test_clipped_draw(self):
        ...
        c.bottom_margin = 60
        c.add_draw_object(
            HorizontalRuler(length=1200, bottom_margin=0, show_borders=True)
        )
        c.add_draw_object(HorizontalSegmentedLine(lengths=600 * [2]))
        title = PageText(
            "A very nice title",
