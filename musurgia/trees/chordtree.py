...
from musurgia.trees.timelinetree import TimelineTree, TimelineDuration
...
class EnsureInitializationMetaclass(ABCMeta):
    def __call__(cls, *args: Any, **kwargs: Any) -> Any:
        ...
        if not getattr(instance, "_chord_initialized", False):
            raise RuntimeError(
                f"The method 'initialize_chord' was not called in {cls.__name__}!"
            )
        return instance
...
class AbstractChordFactory(ABC, metaclass=EnsureInitializationMetaclass):
    ...
    def initiliaze_chord(self) -> None:
        quarter_duration = QuarterDuration(
            convert_duration_to_quarter_duration_value(
                self.get_metronome(), self.get_duration()
            )
        )
    ...
    def update_chord_quarter_duration(self) -> None:
        self._chord.quarter_duration.value = convert_duration_to_quarter_duration_value(
            self.get_metronome(), self.get_duration()
        )
...
class TreeChordFactory:
    __CHORD_UPDATE_METHODS = {
        "_update_chord_quarter_duration",
        "_update_chord_metronome",
    }
    ...
    def _update_chord_quarter_duration(self) -> None:
        self._chord.quarter_duration.value = convert_duration_to_quarter_duration_value(
            self.get_metronome(), self.get_duration().calculate_in_seconds()
        )

    def _update_chord_metronome(self)-> None:
        self._chord.metronome = self.get_metronome()
...
class ChordTree(TimelineTree):
    ...
    def get_chord(self) -> "Chord":
        return self.get_chord_factory().get_chord()
